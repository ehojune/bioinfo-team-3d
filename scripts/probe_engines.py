...
def main() -> None:
    ...
    task = Task(agent_id="probe", prompt="Reply with exactly LABHQ_P1_OK")
    ...
    with tempfile.TemporaryDirectory(prefix="labhq-probe-") as scratch:
        ...
        raw = out / (args.engine + ".raw.jsonl")
        raw_err = out / (args.engine + ".raw.stderr.txt")
        ...
        if args.redact:
            ...
            redact_file(raw, out / (args.engine + ".jsonl"), red)
            redact_file(raw_err, out / (args.engine + ".stderr.txt"), red)
